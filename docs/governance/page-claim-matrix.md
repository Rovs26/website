...
Status: **WEB-3 Home and WEB-4 core public pages approved in bounded scope**
...
CLM-001 through CLM-007 are approved only for the exact WEB-3 and WEB-4
occurrences recorded in their content contracts. CLM-008 has one approved
negative occurrence on `PAG-004`; CLM-009 remains unapproved. Every later page
remains separately gated.

CLM-010 is approved only for the shared header and footer `Sign in` link to
`https://app.kitamo.online/login`. Because the header and footer appear on every
implemented page, PAG-001, PAG-002, PAG-004, and PAG-006 list it. It adds no
page-body wording and does not change any page's primary action.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PAG-001 | `/` | Implemented in WEB-3 | CLM-001, CLM-002, CLM-003, CLM-004, CLM-005, CLM-006, CLM-007, CLM-010 (header and footer only) | `Currently Testing` dominates; no future capability competes with it | Current evidence supports the approved product identity, Android platform, intended audience, testing boundary, local-device wording, and bounded sales/inventory/records summary | Revalidate every used claim before a product release or publication-state change; support, legal, canonical-host, public-download, and indexing decisions remain separate later gates | **Approved for non-indexed WEB-3 Home only.** Exact wording and occurrences are controlled by `docs/content/web-3-homepage-content-contract.md`; this does not activate another route or public launch. |
| PAG-002 | `/about` | Implemented in WEB-4 | CLM-001, CLM-002, CLM-003, CLM-004, CLM-005, CLM-006, CLM-007, CLM-010 (header and footer only) | `Currently Testing` for current product context; `Future Vision` only in a separate noncommittal direction section | Product identity, Android context, intended audience, local-device boundary, bounded record tasks, and lack of broad availability support the exact contract wording | Public organization/legal identity, measured mission or impact claims, specific future capability claims, and assigned content owner remain blocked | **Approved for non-indexed WEB-4 preview only.** Exact occurrences are controlled by `docs/content/web-4/about-content-contract.md`. |
...
| PAG-004 | `/how-it-works` | Implemented in WEB-4 | CLM-001, CLM-002, CLM-003, CLM-004, CLM-006, CLM-007, CLM-008, CLM-010 (header and footer only) | `Currently Testing` at introduction and current workflow; limitations remain current-scope negatives | Evidence supports the approved basic local setup, sales, inventory, record review, no automatic cloud synchronization or backup, and no-online-account wording | Real screenshots, any expanded workflow, deletion guidance, future capability, and public availability require separate approval | **Approved for non-indexed WEB-4 preview only.** Exact occurrences are controlled by `docs/content/web-4/how-it-works-content-contract.md`. |
...
| PAG-006 | `/for-sellers` | Implemented in WEB-4 | CLM-001, CLM-002, CLM-003, CLM-004, CLM-005, CLM-006, CLM-007, CLM-010 (header and footer only) | `Currently Testing` is visible before capability detail | Current evidence and the WEB-4 approval support the exact intended-context examples, conditional record-keeping challenges, local-device boundary, and bounded capability summary | Adoption, universal fit, formal research, business outcomes, and expanded audience or capability claims remain blocked | **Approved for non-indexed WEB-4 preview only.** Exact occurrences are controlled by `docs/content/web-4/for-sellers-content-contract.md`. |
